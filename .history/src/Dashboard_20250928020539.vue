...
onMounted(() => {
  ...
  // 获取文件总量
  fetch('http://localhost:3000/api/file-count', {
    headers: { Authorization: `Bearer ${token}` }
  })
    .then(res => res.json())
    .then(data => {
      if (data.success) fileCount.value = data.count
    })
})

function handleFileCardClick() {
  router.push('/files')
}
...
<template>
  <div class="dashboard-container">
    ...
    <div class="dashboard-stats">
      <div
        class="stat-card file-card"
        :class="{ hover: fileCardHover }"
        @mouseenter="fileCardHover = true"
        @mouseleave="fileCardHover = false"
        @click="handleFileCardClick"
        style="cursor:pointer"
      >
        <div class="stat-title">文件总量</div>
        <div class="stat-value">{{ fileCount }}</div>
        <div v-if="fileCardHover" class="stat-tip">点击查看文件列表</div>
      </div>
      <div class="stat-card">
...
<style scoped>
.file-card.hover {
  box-shadow: 0 0 16px 2px #a8e063;
